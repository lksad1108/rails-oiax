class Program < ApplicationRecord
  ...
  scope :listing, -> {
    # order(aplication_start_time: :desc)
    # joins(:entries)
    left_joins(:entries)
  ...
  scope :published, -> { where("application_start_time <= ?", Time.current)
    .order(application_start_time: :desc)
  }
  ...
  validates :title, presence: true, length: { maxium: 32 }
  validates :description, presence: true, length: { maxium: 800 }
